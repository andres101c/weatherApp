...
const ZipScreen = ({ navigation, route }) => {
    ...
    const fetchClothingSuggestions = (weatherCondition, temp_f) => {
    let suggestions = [];
    
    let itsWarm = false;
    if(temp_f >= 56.0 && temp_f < 130.0){
      itsWarm = true;
    }  

    if (weatherCondition.match('cloudy')) {
      if(itsWarm==true){
        suggestions.push('Shorts', 'T-shirt');
        }  
        else{ suggestions.push('Shirt', 'Light Sweatshirt', 'Pants');   
        }
      } 
    
    else if (weatherCondition.match('Sunny')) {
      if(itsWarm==true){
        suggestions.push('Shorts', 'T-shirt', 'Sunglasses', 'Hat')
      }  else{ 
    ...
    else if (weatherCondition.match('drizzle')) {
      ...
      if(itsWarm==true){
        ...
        suggestions.push('T-Shirt', 'Light Sweatshirt', 'Pants');
      }
    }
    
    else if (weatherCondition.match('Fog')) {     
      if (itsWarm==true) {
        suggestions.push('Shorts', 'T-shirt')
      } else{
        suggestions.push('Be careful driving!', 'Jeans', 'T-shirt', 'Jacket');
      }
    }
    ...
    const updateBackgroundImage = (conditionText) => {
        ...
        if (conditionText.match(/sunny|clear/i)) {
            ...
            setBackgroundImage(is_day ? weatherImages.PartlyCloudy : weatherImages.CloudyNight);
    ...
    try {
        ...
        setWeatherData(data);
        fetchClothingSuggestions(data.current.condition.text, data.current.temp_f);
